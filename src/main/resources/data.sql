-- Sample movie data with image paths (using individual inserts for better compatibility)

-- Movie 1
INSERT INTO movie (title, director, release_date, duration_minutes, genre, description, image_path) 
SELECT 'The Shawshank Redemption', 'Frank Darabont', '1994-09-23', 142, 'Drama', 'Two imprisoned men bond over a number of years, finding solace and eventual redemption through acts of common decency.', 'shawshank.jpg'
WHERE NOT EXISTS (SELECT 1 FROM movie WHERE title = 'The Shawshank Redemption');

-- Movie 2
INSERT INTO movie (title, director, release_date, duration_minutes, genre, description, image_path) 
SELECT 'The Godfather', 'Francis Ford Coppola', '1972-03-24', 175, 'Crime, Drama', 'The aging patriarch of an organized crime dynasty transfers control of his clandestine empire to his reluctant son.', 'godfather.jpg'
WHERE NOT EXISTS (SELECT 1 FROM movie WHERE title = 'The Godfather');

-- Movie 3
INSERT INTO movie (title, director, release_date, duration_minutes, genre, description, image_path) 
SELECT 'The Dark Knight', 'Christopher Nolan', '2008-07-18', 152, 'Action, Crime, Drama', 'When the menace known as the Joker wreaks havoc and chaos on the people of Gotham, Batman must accept one of the greatest psychological and physical tests of his ability to fight injustice.', 'dark_knight.jpg'
WHERE NOT EXISTS (SELECT 1 FROM movie WHERE title = 'The Dark Knight');

-- Movie 4
INSERT INTO movie (title, director, release_date, duration_minutes, genre, description, image_path) 
SELECT 'Pulp Fiction', 'Quentin Tarantino', '1994-10-14', 154, 'Crime, Drama', 'The lives of two mob hitmen, a boxer, a gangster and his wife, and a pair of diner bandits intertwine in four tales of violence and redemption.', 'pulp_fiction.jpg'
